...
description: Aprenda a utilizar la corrección de sesgo para corregir defectos de cocción al utilizar un flujo de trabajo de alto a bajo nivel de poly en Substance 3D Painter.
...
<table>
  <tr style="border: 0;">
    ...
    <td style="border: 0; width: 65%" valign="top">A veces, al hornear a baja polietileno de un modelo de alta polietileno, es posible que los detalles aparezcan deformados o sesgados. Esto suele ocurrir cuando las normales de la jaula y las normales de la superficie no se alinean bien. El horneado automático proyecta el alto-poli sobre el bajo-poli basándose en estos valores normales, por lo que si son incorrectos, el horneado produce resultados deficientes.<br>Afortunadamente, la corrección de sesgo (o la asignación de sesgo) está disponible para ayudar a corregir este tipo de artefacto.La corrección de sesgo de <br>Skew le permite pintar valores directamente en la malla de baja densidad para redirigir la proyección utilizada durante el horneado sin necesidad de crear una jaula personalizada.</td>
  </tr>
</table>

>[!NOTE]
>
> La corrección de sesgo se pinta dentro del **modo de cocción** y se almacena por conjunto de texturas.

## Pintar correcciones de sesgo

La pintura de corrección de sesgo le permite ajustar manualmente las normales de la superficie de la malla específicamente para el horneado. Aunque puedes pintar correcciones de sesgo sin hornear, puede ayudar a [hornear primero tus mapas de malla](how-to-bake-mesh-maps.md).

![](../assets/baking/mode_select_buttons.png)

*Cambiar al modo de procesamiento para tener acceso a la configuración de corrección de sesgo.*

>[!IMPORTANT]
>
> La pintura con corrección de sesgo requiere los siguientes ajustes:
>
> * Es necesario seleccionar una escena High Poly. La pintura de sesgo solo está disponible al hornear de alto a bajo poli; Si se ha marcado **Usar malla de poli baja como malla de poli alta**, la pintura de corrección de sesgo **no** estará disponible.
> * **La jaula** se debe establecer en **basada en la distancia**.
> * **Se debe comprobar el promedio de normales**.

Con la configuración anterior, puedes hacer clic en **Corrección de sesgo de pintura** en el **Panel de configuración común** para empezar a pintar. Cuando entras por primera vez en el modo de pintura de corrección de sesgo, **Auto-rebake** se activará automáticamente para el canal normal. Si lo prefieres, puedes desactivar la **reconversión automática** o cambiar el canal seleccionado en el [**panel de marcadores de mapas de malla**](../interface/baking-panels/mesh-map-bakers.md).
...
* Puedes cambiar entre **Pincel**, **Borrador** y **Relleno poligonal** en la barra de herramientas, o usar el [método abreviado de teclado](../interface/settings/shortcuts.md) estándar del modo de pintura.
...
**La rehorneación automática** está optimizada para intentar limitar la rehorneación al área afectada por cada trazo de pincel al pintar correcciones de sesgo. Al pintar un trazo, **Volver a hacer automáticamente** dibuja un cuadro delimitador alrededor del trazo en el espacio UV y vuelve a hacer todo lo que hay dentro del cuadro. Esto significa que si el trazo solo cubre una pequeña sección de espacio UV, solo se rehorneará una pequeña área, haciendo que la operación sea muy eficiente.
...
Con **Alignment** establecido en **UV**, es más difícil pintar trazos suaves en las costuras de la Isla de UV; sin embargo, esto suele ser menos importante al pintar correcciones de sesgo que al texturizar la malla.

>[!NOTE]
>
> Los parámetros de **Brush** y **Eraser** se almacenan por separado. Para maximizar el rendimiento de ambas herramientas, deberá establecer la **alineación** para cada una de ellas de forma individual.

## Correcciones de sesgo y la pila de deshacer

El horno y la pintura comparten un único historial de deshacer. El cambio entre el modo de cocción y el modo de pintura es en sí mismo un paso que se puede deshacer, y la activación o desactivación de la corrección de sesgo también se puede deshacer. Cuando se deshace una acción de horneado en el modo de pintura, el modo de horneado se vuelve a abrir automáticamente antes de que se deshagan esos pasos, por lo que nunca se deshace una acción fuera del modo en el que ocurrió.