---
helpx_url: "https://helpx.adobe.com/es/substance-3d-painter/scripting-and-development/api-reference/shader-api.html"
breadcrumb-title: ''
...
En Substance Painter, puede escribir sus propios sombreadores en *GLSL*. Te permitimos escribir solo una *porción* del sombreador del fragmento, que a veces se denomina *sombreador de superficie*. Sin más preámbulos, presentemos el sombreador de superficie Substance Painter &quot;Hello world&quot;:
...
Ahora, si guardas este fragmento en un archivo *.glsl* y lo cargas en Substance Painter soltándolo en la pestaña sombreador de tu estantería, ahora puedes usarlo y ver un hermoso color rosa uniforme en tu malla.

## Sombreador de superficie

* [surface-sombreador.glsl](shaders-shader-api/surface-shader-shader-api.md)
...
En algunos casos, puede que desee utilizar una configuración de procesamiento específica (sacrificio, fusión, localidad de muestreo, etc.) para un efecto. Algunos estados de renderizado se muestran y se pueden definir en el sombreador. A continuación se muestra una lista exhaustiva de todos los estados de procesamiento expuestos :

* [all-rendering-states-params.glsl](parameters-shader-api/all-rendering-states-params-shader-api.md)

## Ajustes personalizados (¿o cómo modifico mi sombreador?)

Es habitual tener ajustes personalizados en un sombreador. Para ello, hemos introducido una forma de especificar ajustes personalizados en los sombreadores de Substance Painter. A continuación se muestra una lista exhaustiva de todos los tipos de ajustes de sombreador personalizados :
...
* [lib-emisivo.glsl](libraries-shader-api/lib-emissive-shader-api.md) : contiene ayudantes de propiedades de emisivo
* [lib-env.glsl](libraries-shader-api/lib-env-shader-api.md) : contiene ayudantes relacionados con el mapa de entorno
* [lib-normal.glsl](libraries-shader-api/lib-normal-shader-api.md) : contiene ayudantes relacionados con el mapa de normales (y mapa de normales generado por mapa de height)
...
* [lib-sparse.glsl](libraries-shader-api/lib-sparse-shader-api.md) : contiene ayudantes de muestreo de textura dispersos y seguros
...
* **interfaz de usuario personalizada**: Reemplace la interfaz de usuario de parámetros de sombreador estándar por una vista personalizada escrita como módulo QML (consulte la documentación de scripts). La ruta de acceso puede ser absoluta o relativa a una de las carpetas *custom-ui* de la estantería.
* **mdl**: definir el material del molde de Iray que se utilizará con el sombreador. La sintaxis de la ruta es la siguiente: *mdl::folder1::folder2::mdl\_filename::material\_name* donde *folder1::folder2::mdl\_filename* es la ruta de acceso dentro de una de las carpetas *mdl* del estante a un archivo mdl y *::material\_name* es el nombre de un material declarado dentro de este archivo mdl. (ej.: &quot;mdl&quot; : &quot;mdl::alg::materials::físicamente\_metálico\_roughness::físicamente\_metálico\_roughness&quot;)

## Ejemplos de sombreadores (¡sí, por fin!)

Para probar lo que parece un sombreador real, aquí hay algunos ejemplos de sombreador, ordenados por complejidad creciente:

* [pixelated.glsl](shaders-shader-api/pixelated-shader-api.md) : un sombreador pixelado
* [toon.glsl](shaders-shader-api/toon-shader-api.md) : un sombreador toon
...
El Control dinámico de capas de materiales es un flujo de trabajo específico en el que los materiales se mezclan dentro de un sombreador y el usuario puede editar dinámicamente las máscaras de fusión en Substance Painter. Para activar este flujo de trabajo, existen dos nuevas funcionalidades:

* declarar pilas editables a partir de una definición de sombreador: [capas\_declare\_stacks.glsl](parameters-shader-api/layering-declare-stacks-shader-api.md)
* enlazar materiales como parámetros de sombreador: [capas\_bind\_materials.glsl](parameters-shader-api/layering-bind-materials-shader-api.md)
